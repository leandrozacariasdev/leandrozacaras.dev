...
export default function Navbar({ showBack = false }: NavbarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-sm border-b border-zinc-200 dark:border-zinc-800">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 px-4 py-3">
          <div className="flex flex-col gap-3 text-sm">
            ...
            {mainLinks.map((link) => (
              <Link key={link.href} href={link.href} className="py-2" onClick={() => setMobileMenuOpen(false)}>
                {link.label}
              </Link>
            ))}
            {moreLinks.map((link) => (
            ...
            <Link href="/#contato" className="py-2" onClick={() => setMobileMenuOpen(false)}>
              {navLabels.contact}
            </Link>
            <div className="flex gap-4 pt-2 border-t border-zinc-200 dark:border-zinc-800">
              <button onClick={toggleLocale} className="flex items-center gap-1 py-2">
                <Globe className="w-4 h-4" /> {locale === 'pt-BR' ? 'Português' : 'English'}
              </button>
              <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="flex items-center gap-1 py-2">
                {mounted && (theme === 'light' ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />)} {locale === 'pt-BR' ? 'Tema' : 'Theme'}
              </button>
            </div>
          </div>
        </div>
